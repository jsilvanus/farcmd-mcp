...
async function sshPage() {
  ...
  const targetRows=targets.map(t=>{const v=t.verifier?.status??'unavailable'; const key=keys.find(k=>k.id===t.sshKeyId);
    return row(
      ...
      '<div class="pills">'+(v==='active'?pill('Verifier active','ok'):v==='pending'?pill('Verifier not activated','warn'):pill('No verifier: levels 3–5 blocked','bad'))+(t.verifier?.lastError?pill('Last verification failed','bad'):'')+(t.enabled?'':pill('Disabled'))+'</div>',
  ...
  on('[data-delete-key]',async b=>{if(confirm('Delete this SSH key?')){try{await api('/api/ssh/keys/'+b.dataset.deleteKey,{method:'DELETE'});}catch(err){alert((err as Error).message);} sshPage();}});
