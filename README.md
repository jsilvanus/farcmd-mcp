...
- Install or repair the verifier on the SSH page: automatically (the master key needs root on the target: the root account, the account's sudo password entered once in the UI and never stored, or passwordless sudo), or with a one-time **manual root install script**, which works without any master key. Target requirements: `python3` and `sudo`.
