...
export class SqliteCommandStore {
  ...
  list(userId:string):CommandRecord[]{const rows=this.db.prepare('SELECT id,user_id,target_id,name,description,type,content,command_sha256,level,enabled,show_output_on_approval,verify_integrity,created_at,updated_at FROM commands WHERE user_id=? ORDER BY name,id').all(userId) as any[];return rows.map(this.map).filter((c):c is CommandRecord=>c!==undefined);}
  get(userId:string,id:string):CommandRecord|undefined{return this.map(this.db.prepare('SELECT id,user_id,target_id,name,description,type,content,command_sha256,level,enabled,show_output_on_approval,verify_integrity,created_at,updated_at FROM commands WHERE user_id=? AND id=?').get(userId,id) as any);}
  create(r:CommandRecord):void{this.db.prepare('INSERT INTO commands (id,user_id,target_id,name,description,type,content,command_sha256,level,enabled,show_output_on_approval,verify_integrity,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)').run(r.id,r.userId,r.targetId,r.name,r.description,r.type,r.content,hashCommandContent(r.type,r.content),r.level,r.enabled?1:0,r.showOutputOnApproval?1:0,r.verifyIntegrity?1:0,r.createdAt,r.updatedAt);}
  update(r:CommandRecord):void{this.db.prepare('UPDATE commands SET target_id=?,name=?,description=?,type=?,content=?,command_sha256=?,level=?,enabled=?,show_output_on_approval=?,verify_integrity=?,updated_at=? WHERE user_id=? AND id=?').run(r.targetId,r.name,r.description,r.type,r.content,hashCommandContent(r.type,r.content),r.level,r.enabled?1:0,r.showOutputOnApproval?1:0,r.verifyIntegrity?1:0,r.updatedAt,r.userId,r.id);}
