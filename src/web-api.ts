...
import { isActiveUser, type AuthStore, type UserStore, type WebSessionRecord, type WebSessionStore } from './storage/interface.js';
...
const { utils }=ssh2;
import { SqliteCommandInstallationStore, SqliteRemoteCapabilityLedgerStore, type CommandInstallationRecord } from './storage/command-installations.js';
...
declare module 'fastify' { interface FastifyRequest { webSession?:WebSessionRecord|undefined; auditUserId?:string|undefined; auditError?:string|undefined; } }
...
export async function mountWebApi(app:FastifyInstance, users:UserStore, sessionStore:AuthStore&WebSessionStore&{getDatabase():DatabaseSync}, publicUrl=process.env.MCP_PUBLIC_URL ?? `http://localhost:${process.env.PORT ?? '5999'}`): Promise<void> {
  ...
  // The session is resolved once here and reused by requireUser.
  ...
  app.get('/api/oauth/grants',async(request,reply)=>{const user=await requireUser(request,reply,users,sessions);if(!user)return;return {grants:sessionStore.listOAuthGrants(user.id).map((g:any)=>({...g,revoked:!!g.revokedAt}))};});
