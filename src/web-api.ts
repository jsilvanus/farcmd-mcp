...
export async function mountWebApi(app:FastifyInstance, users:UserStore, sessionStore:WebSessionStore): Promise<void> {
  ...
  app.patch('/api/ssh/targets/:id',async (request,reply)=>{
    ...
    if(!next.name||!next.hostname||!next.username||!Number.isInteger(next.port)||next.port<1||next.port>65535||!ssh.getKey(user.id,next.sshKeyId)) return reply.code(400).send({error:'Invalid SSH target data'}); if(next.sshKeyId!==current.sshKeyId&&commandKeys.list(user.id).some(k=>k.targetId===id)) return reply.code(409).send({error:'This target has installed per-command SSH keys. Remove them before changing its master key.'}); ssh.updateTarget(next); return {target:ssh.getTarget(user.id,id)};
  });
