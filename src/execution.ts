...
export class SqliteExecutionStore {
  ...
  consume(token:string):PendingExecution|undefined{const p=this.get(token);if(!p)return undefined;this.db.prepare("UPDATE pending_executions SET status='consumed' WHERE token=? AND status='pending'").run(token);return p;}
