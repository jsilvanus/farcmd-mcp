...
function consentPage(session:string,userName:string,clientName:string,current:number[],permanent5:boolean):string{
 ...
 return page('Authorize farcmd','<h1>Authorize farcmd</h1><p><strong>'+escapeHtml(clientName)+'</strong> wants access as <strong>'+escapeHtml(userName)+'</strong>.</p><p>Select which command levels this OAuth source may execute.</p><form method="post" action="/oauth/authorize"><input type="hidden" name="session" value="'+escapeHtml(session)+'">'+levels+(permanent5?'<p class="muted">Level 5 has been permanently prohibited for this OAuth source.</p>':'<div class="level"><label><input type="checkbox" name="permanentLevel5" value="yes"> Permanently prohibit level 5 for this OAuth source</label></div>')+'<button type="submit" name="action" value="approve">Approve selected levels</button><button class="secondary" type="submit" name="action" value="deny">Deny</button></form>');
}
...
export async function mountAuthorizationServer(app:FastifyInstance,issuer:string,resource:string,secret:Uint8Array,authStore:AuthStore,users:UserStore):Promise<void>{
 ...
 app.post('/oauth/authorize',async(request,reply)=>{
  ...
  if(existing?.level5PermanentlyDenied&&(!permanent5||allowed.includes(5))){return reply.code(400).type('text/html').send(page('Permission denied','<h1>Level 5 is permanently prohibited for this OAuth source.</h1>'));}
  ...
  authStore.upsertOAuthGrant(user.id,q.client_id!,metadata.client_name,allowed,permanent5||!!existing?.level5PermanentlyDenied);
